...
\lstdefinelanguage{XML}
{
  ...
  morekeywords={xmlns,version,type,android,label,theme,allowBackup,usesCleartextTraffic}% list your attributes here
}
\lstdefinelanguage{YAML}
{
  ...
  stringstyle=\color{scw-teal-darker}\bfseries,
  keywordstyle=\color{scw-orange-darker}\bfseries,
  morekeywords={search,methodcall,name,type,args,containsUntrustedInput,trustedSources,1}% list your attributes here
}
...
\newacronym{cve}{CVE}{Common Vulnerabilities and ExpPrivacy and trustosures}
...
\newglossaryentry{flaw}
{
    name=flaw,
    plural={flaws},
    description={A mistake in the design of the application that can lead to unwanted behaviour. They are mistakes in the functional structure of the application that are much harder to detect automatically than security bugs}
}
...
\parskip 1mm
\input{01-frontmatter/abstract-dutch}
\parskip 0mm
\newpage
\glsresetall
...
\todo[inline]{also check for indirect wording: "this means that X" -> X}
\todo[inline]{rule -> recipe where needed}
\todo[inline]{his/her -> their}
\todo[inline]{not "very", not "much", no contractions (e.g. don't --> do not)}
\todo[inline]{we -> I where appropriate}
\todo[inline]{quickfix or quick fix --> quick-fix}
\todo[inline]{check thousands notation throughout document: use commas, not dots}
...
\todo[inline]{probability vs likelihood}
\todo[inline]{developer team -> development team}
\todo[inline]{code base, or codebase?}

\input{02-vision/vision}
\input{03-education/education}
\input{04-tools/tools}

\appendix
\input{06-appendices/00-scoring}
% it's just too long, and nobody cares anyway
%\input{06-appendices/01-categories}
\input{06-appendices/04-pairups}
\input{06-appendices/06-its-metrics}
\input{06-appendices/03-patterns}
\input{06-appendices/02-scopes}
\input{06-appendices/07-battlecards}
