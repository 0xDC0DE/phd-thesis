...
Currently, there are about 15,000 challenges completed every day, that is about one challenge every five seconds and this number is only expected to go up.
...
The \textit{purpose} is to determine if they can be used to improve the efficiency of the ability computations without significant loss in accuracy.
...
From that point forward, the approximation methods are used.
...
The fixed step size adjustment becomes excessively inaccurate after only 15 to 20 challenges after the initial calibration.
...
The mean error of 10\% is acceptable to be used in the \gls{its}, as will become evident in a following experiment in Section~\ref{sec:eval-learning}.
