...
Semgrep allows easy customization of rules through a \gls{yaml} syntax taht is very similar to that of Sensei.
...
Checkmarx claims flexible rules lead to higher accuracy, and have a very extensive Query Language (CxQL)~\cite{cxaudit} to create and adapt rules.  
Its custom query language CxQuery uses regular Java syntax and is easy to understand.
...
It seems that the rules in CxQuery can be built to ignore context and enforce secure coding guidelines in line with the paved path methodology.
...
\Gls{fod}~\cite{fod} provides similar features to FSCA but through a web portal, Micro Focus calls this Application Security as a Service.

\Gls{fsa}~\cite{fsa} is a plugin currently available for Eclipse and Visual Studio. It allows security scans in the IDE, where the rule set is tuned such that the longest analyses are disabled by default. The scan can take several minutes during which the developer cannot make any code changes. This is still quite long compared to the real-time results of Sensei and might inhibit developers from requesting scans frequently during development. 
...
Each tool provides detailed descriptions of vulnerabilities, which focus on explaining the vulnerabilities in detail, in part by providing examples of insecure code. In a second tab, FOD provides a description and code examples to resolve the vulnerability. All tools provide links to reference material and to recommended solutions. FOD also provides links to Secure Code Warrior to provide training on a specific vulnerability. The information provided is very similar to the different descriptions of Sensei, but the focus is more on explaining the vulnerability first instead of the solution.
}{SAST}{Test}{Minutes}{Description}


\battlecard{Veracode}{\url{https://www.veracode.com/}}{
\label{bc:veracode}
Veracode offers two tools. 
\Gls{vsa} is a SaaS platform that allows the developer to upload their code to be analyzed. \Gls{vg} is an \gls{ide} plugin that performs lightweight versions of the same analyses.

The tools perform static analysis scans on compiled bytecode of web applications in 23 programming languages.
...
In my opinion, Veracode is not suited as a developer tool.
...
}{SAST}{Test}{Hour}{Descriptions, consultations}